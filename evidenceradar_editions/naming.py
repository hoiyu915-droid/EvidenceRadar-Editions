...
def infer_period_kind(start: date, end: date) -> str:
    ...
    if (
        ...
        and end.day == _last_day_of_month(start)
    ):
        return "month"
    if (
        start.weekday() == 0
        and end == start + timedelta(days=6)
        and start.isocalendar()[:2] == end.isocalendar()[:2]
    ):
        return "week"
    return "range"
...
def resolve_period_kind(start: date, end: date, requested: str = "auto") -> str:
    ...
    if requested != inferred and requested != "range":
        raise ValueError(
            f"period {start.isoformat()}..{end.isoformat()} is {inferred}, not {requested}"
        )
    return requested
...
def build_identity(
    *,
    ...
    period_kind_requested: str = "auto",
    revision: int = 1,
) -> EditionIdentity:
