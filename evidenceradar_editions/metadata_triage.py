...
def triage_article(
    ...
    *,
    ...
    if matched:
        ...
        primary_index = len(validated["signals"])
        primary = {
            "tier": default["tier"],
...
def triage_sort_key(article: Mapping[str, Any]) -> tuple[Any, ...]:
    ...
    return (
        ...
        int(triage.get("signal_order") or 9999),
        -int(triage.get("identifier_count") or 0),
        -int(triage.get("source_count") or 0),
...
def _select_tier(
    grouped: Mapping[tuple[int, str], deque[dict[str, Any]]],
    ...
    *,
    ...
    while len(selected) < remaining_limit and any(grouped[key] for key in keys):
        progressed = False
        for key in keys:
            queue = grouped[key]
            if not queue or len(selected) >= remaining_limit:
                continue
            attempts = len(queue)
    ...
    if len(selected) < remaining_limit:
        ...
        fill = remaining[: remaining_limit - len(selected)]
...
def select_triaged_projection(
    ...
    *,
    ...
    enriched = [
        ...
        else enrich_article_with_triage(article, policy=validated)
    ...
    threshold = float(
        validated["selection"]["near_duplicate_jaccard_threshold"]
    )
    bucket_order = list(validated["selection"]["bucket_order"])
    bucket_rank = {value: index for index, value in enumerate(bucket_order)}
    tier_rank = {tier: index for index, tier in enumerate(ALLOWED_TIERS)}
    grouped: defaultdict[
        tuple[int, str], deque[dict[str, Any]]
    ] = defaultdict(deque)
    for article in enriched:
        ...
        grouped[(tier_rank.get(tier, len(ALLOWED_TIERS)), *key)].append(
            article
        )
    ...
    for tier_index in range(len(ALLOWED_TIERS)):
        ...
        if not tier_keys or len(selected) >= limit:
            continue
        tier_selected = _select_tier(
            grouped,
            tier_keys,
