...
def _patch_portal_home(html: str, summary: dict[str, Any]) -> str:
    ...
    tiers = (summary.get("canonical_triage_counts") or {}).get("by_tier") or {}
    priority = int(tiers.get("ALERT", 0)) + int(tiers.get("HIGH", 0))
    panel = (
        ...
        f'目前 {int(summary.get("canonical_article_count") or 0):,} 筆 canonical records '
        f'全部完成 title／bibliographic metadata 分流；{priority:,} 筆為 ALERT／HIGH。'
        f'預設 browse/search 載入 {int(summary.get("default_projected_article_count") or 0):,} 筆，'
        f'另有 {int(summary.get("default_omitted_article_count") or 0):,} 筆只從預設 payload 省略、'
        '未從 canonical archive 刪除。'
