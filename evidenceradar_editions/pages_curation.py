...
_EDITORIAL_RE = re.compile(
    r"^(?:(?:guest\s+)?editorial(?:\s*:|\s*$)|preface\b|introduction\s+(?:to|for)\s+(?:the\s+)?(?:special|themed)\s+issue\b)",
    re.IGNORECASE,
)
