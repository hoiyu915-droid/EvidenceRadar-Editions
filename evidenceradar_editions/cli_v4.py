...
def _spec_from_args(args: argparse.Namespace) -> EditionSpec:
    ...
    if getattr(args, "journal_slug", None):
        journal_record = get_journal(
            str(args.journal_slug),
            catalog_root=Path(args.catalog_root),
            require_enabled=not bool(getattr(args, "allow_planned", False)),
        )
        registry_defaults = journal_spec_defaults(journal_record)

    journal = args.journal or payload.get("journal") or registry_defaults.get("journal")
    if not journal:
        raise ValueError("journal is required; pass --journal or --journal-slug")
    start = args.start or payload.get("start_date")
    end = args.end or payload.get("end_date")
    if not start or not end:
        raise ValueError("start and end dates are required")

    if args.issn is not None:
        issn = args.issn
    elif payload.get("issn") is not None:
        issn = payload.get("issn")
    else:
        issn = registry_defaults.get("issn")

    slug = (
        args.slug
        or payload.get("slug")
        or registry_defaults.get("slug")
        or slugify(str(journal))
    )
    sources_raw = (
        args.sources
        or payload.get("sources")
        or registry_defaults.get("sources")
        or list(ALLOWED_SOURCES)
    )
...
def _add_scope_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--journal-slug",
        help="resolve journal name, ISSN and direct acquisition sources from catalog/journals.json",
    )
    parser.add_argument("--catalog-root", type=Path, default=Path("catalog"))
    parser.add_argument(
        "--allow-planned",
        action="store_true",
        help="allow a registry entry with enabled=false",
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    run.add_argument("--translation-request", type=Path)

    journals = sub.add_parser(
        "journals",
        help="list the local Editions journal registry without reading EvidenceRadar",
    )
    journals.add_argument("--catalog-root", type=Path, default=Path("catalog"))
    journals.add_argument("--status", choices=("active", "planned", "retired"))
    journals.add_argument("--category")
    journals.add_argument("--publisher")
    journals.add_argument("--enabled-only", action="store_true")
    ...
    request = sub.add_parser(
        "translation-request",
        help="create a hash-bound zh-TW translation request",
    )
    ...
    apply_translation = sub.add_parser(
        "apply-translation",
        help="apply a hash-bound zh-TW response",
    )
    ...
    publish = sub.add_parser(
        "publish",
        help="store a validated edition in sharded canonical storage",
    )
    publish.add_argument("--bundle-dir", type=Path, required=True)
    publish.add_argument("--editions-root", type=Path, default=Path("editions"))
    publish.add_argument(
        "--archive-root",
        type=Path,
        help="deprecated v0.2 compatibility mode; stores the legacy HTML-heavy archive",
    )
    publish.add_argument("--allow-untranslated", action="store_true")

    pages = sub.add_parser(
        "build-pages",
        help="build the static multi-journal GitHub Pages portal",
    )
    pages.add_argument("--editions-root", type=Path, default=Path("editions"))
    pages.add_argument("--archive-root", type=Path, help="deprecated v0.2 compatibility input")
    pages.add_argument("--catalog-root", type=Path, default=Path("catalog"))
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "journals":
            ...
            print(json.dumps({"journal_count": len(items), "journals": items}, ensure_ascii=False))
            return 0
        ...
        if args.command == "translation-request":
            ...
            print(json.dumps({"edition_id": request["edition_id"], "items": request["item_count"], "output": str(args.output)}, ensure_ascii=False))
            return 0
        ...
        if args.command == "apply-translation":
            ...
            enriched = apply_translation_response(run, response, require_complete=not args.allow_partial)
            manifest = write_bundle(enriched, args.output_dir)
            errors = validate_bundle(args.output_dir, require_zh_tw=not args.allow_partial)
            if errors:
                _print_errors(errors)
                return 1
            print(json.dumps({"edition_id": enriched["edition_id"], "translated_articles": enriched["counts"]["translated_articles"], "manifest": manifest["manifest_name"], "output_dir": str(args.output_dir)}, ensure_ascii=False))
            return 0

        if args.command == "publish":
            if args.archive_root is not None:
                target = publish_legacy_bundle(args.bundle_dir, args.archive_root, require_zh_tw=not args.allow_untranslated)
                mode = "legacy_archive_v02"
            else:
                target = store_bundle(args.bundle_dir, args.editions_root, require_zh_tw=not args.allow_untranslated)
                mode = "canonical_store_v3"
            print(json.dumps({"published": str(target), "storage_mode": mode}, ensure_ascii=False))
            return 0
        ...
        if errors:
            _print_errors(errors)
            return 1
        print(json.dumps({"edition_id": run["edition_id"], "articles": run["counts"]["articles"], "output_dir": str(args.output_dir), "html": manifest["files"]["report_html"]["name"], "json": manifest["files"]["edition_json"]["name"], "manifest": manifest["manifest_name"], "upstream_radar_commit": manifest.get("upstream_radar_commit")}, ensure_ascii=False))
        return 0
