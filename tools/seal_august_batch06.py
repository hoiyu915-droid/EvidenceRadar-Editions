...
EXPECTED = {
    ...
    "nutrition-research": 15,
    "sports-medicine-and-health-science": 2,
    "the-lancet-regional-health-europe": 33,
    "the-lancet-regional-health-western-pacific": 16,
}
MANUAL_EXCLUDE = {
    "doi:10.1016/s0271-5317(26)00094-1",  # Editorial office and Board Members
    "doi:10.1016/s0271-5317(26)00095-3",  # Graduate student award notice
}
SIMPLIFIED_SENTINELS = set("这为与从体国发后疗应门东会术网动数据压")
...
def load_translations() -> dict[str, str]:
    result: dict[str, str] = {}
    for path in sorted(Path("translation-work").glob("*.ndjson")):
        for lineno, line in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
            ...
            if any(ch in title for ch in SIMPLIFIED_SENTINELS):
                bad = sorted({ch for ch in title if ch in SIMPLIFIED_SENTINELS})
                raise RuntimeError(f"possible simplified-Chinese contamination {bad}: {path}:{lineno}")
            if cid in result:
...
def excluded_ids() -> set[str]:
    excluded: set[str] = set(MANUAL_EXCLUDE)
...
def prepare_run(slug: str, excluded: set[str]) -> dict:
    ...
    if len(articles) != EXPECTED[slug]:
        raise RuntimeError(f"{slug}: expected {EXPECTED[slug]} retained articles, got {len(articles)}")
...
def main() -> None:
    ...
    if len(wanted) != 86:
        raise RuntimeError(f"expected 86 retained articles, got {len(wanted)}")

    workspace = Path(tempfile.mkdtemp(prefix="batch06-"))
    try:
        for slug in SLUGS:
        ...
        if int(search.get("article_count") or 0) < 1079:
            raise RuntimeError("Pages search index did not grow to the expected minimum")
        print(json.dumps({"batch_articles": sum(EXPECTED.values()), "new_monthly_counts": EXPECTED, "search_articles": search.get("article_count")}, ensure_ascii=False, indent=2))
    finally:
