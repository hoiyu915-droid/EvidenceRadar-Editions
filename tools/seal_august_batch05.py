...
EXPECTED = {
    ...
    "computers-in-human-behavior-artificial-humans": 31,
    "computers-in-human-behavior-reports": 103,
    "international-journal-of-human-computer-studies": 9,
}
MANUAL_EXCLUDE = {
    "doi:10.1016/j.ijhcs.2026.103904",  # special-issue preface
}
# A narrow high-signal guard against accidental Simplified-Chinese input in zh-TW titles.
SIMPLIFIED_SENTINELS = set("学与为这国网术语发开参应现过证评动资质机实验认数类长线说")
...
def load_translations() -> dict[str, str]:
    ...
    for path in sorted(Path("translation-work").glob("*.ndjson")):
        for lineno, line in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
            ...
            bad = sorted(set(title) & SIMPLIFIED_SENTINELS)
            if bad:
                raise RuntimeError(
                    f"possible Simplified-Chinese contamination in {cid}: {''.join(bad)}"
                )
            if cid in result:
...
def excluded_ids() -> set[str]:
    ...
    flags = load_json(Path("probe/flags.json"))
    excluded.update(
        str(item.get("canonical_id"))
        for item in (flags.get("items") or [])
        if isinstance(item, dict) and item.get("canonical_id")
    )
    replacement = load_json(Path("probe/replacement-summary.json"))
    excluded.update(
        str(item.get("canonical_id"))
        for item in (replacement.get("flags") or [])
        if isinstance(item, dict) and item.get("canonical_id")
    )
    return excluded
...
def main() -> None:
    ...
    if len(wanted) != 148:
        raise RuntimeError(f"expected 148 retained articles, got {len(wanted)}")

    workspace = Path(tempfile.mkdtemp(prefix="batch05-"))
    try:
        for slug in SLUGS:
        ...
        if int(search.get("article_count") or 0) < 993:
