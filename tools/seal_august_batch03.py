...
EXPECTED = {
    "artificial-intelligence-in-medicine": 12,
    "gait-and-posture": 3,
    "human-movement-science": 6,
    "journal-of-clinical-epidemiology": 45,
    "physical-therapy-in-sport": 2,
}
MANUAL_EXCLUDE = {
    "doi:10.1016/j.jclinepi.2026.112403",  # Editors' Choice August 2026
}
...
def main() -> None:
    ...
    if len(wanted) != 68:
        raise RuntimeError(f"expected 68 retained articles, got {len(wanted)}")

    workspace = Path(tempfile.mkdtemp(prefix="batch03-"))
    try:
        for slug in SLUGS:
        ...
        if int(search.get("article_count") or 0) < 636:
